...
void PlotClear()
{
    ...
    UTFT_setColor(g_background_color);
...
void PlotInit(int x, int y, int dx, int dy)
{
    ...
    uint8_t cx = UTF_StringWidth("0");
    uint8_t cy = UTF_Height();
...
void PlotSetAxis(float xmin, float xmax, float ymin, float ymax)
{
    ...
    UTFT_setBackColor(g_background_color);
    PlotSetFont();
    uint8_t fontx = UTF_StringWidth("0");
    uint8_t fonty = UTF_Height();
